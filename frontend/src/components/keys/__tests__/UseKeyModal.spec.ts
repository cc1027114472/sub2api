...
describe('UseKeyModal', () => {
  ...
  it('renders Codex custom provider setup through the Grok Responses gateway', async () => {
    ...
    let codeBlocks = wrapper.findAll('pre code').map((code) => code.text())
    const configToml = codeBlocks.find((content) => content.includes('[model_providers.gateway_grok]'))
    expect(configToml).toBeDefined()
    expect(configToml).toContain('model_provider = "gateway_grok"')
    expect(configToml).toContain('model = "grok-4.5"')
    expect(configToml).toContain('model_context_window = 1000000')
    expect(configToml).toContain('base_url = "https://example.com/v1"')
    expect(configToml).toContain('env_key = "GATEWAY_API_KEY"')
    ...
    expect(codeBlocks).toContain('export GATEWAY_API_KEY="sk-grok-codex-test"')
    ...
    expect(codeBlocks).toContain('$env:GATEWAY_API_KEY="sk-grok-codex-test"')
  })
