export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      grok: {
        description:
          '配置 Grok CLI、Claude Code、Codex 或 OpenCode，让请求通过当前 英国api.cc Grok 分组发送。文本模型走 Responses；图片/视频使用 Imagine 模型 ID 与媒体端点。',
        claudeDescription: '配置 Claude Code，让 Messages API 请求通过当前 英国api.cc Grok 分组发送。',
        codexDescription: '配置 Codex，让 Responses API 请求通过当前 英国api.cc Grok 分组发送。',
        configTomlHint:
        ...
        codexConfigTomlHint:
          'Codex 官方：wire_api 仅支持 "responses"；优先 env_key，勿与 experimental_bearer_token 混用；非 OpenAI 网关默认 supports_websockets = false（英国api.cc 仍可接客户端 WS 并桥接到 HTTP/SSE）。合并前备份 ~/.codex/config.toml。',
        note:
    ...
    mowanNotInstalled:
      '未检测到墨顽 Agent，可尝试手动启动或在浏览器直接打开本地控制台 (http://127.0.0.1:3090)。',
    ccsClientSelect: {
      title: '选择客户端',
  ...
  agentDownload: {
    ...
    releaseDate: '发布日期',
    fileSize: '安装包大小',
    envIncluded: '自带独立运行环境，无需安装 Node.js 或繁琐配置，下载后一键极速运行。',
    ...
    windowsDesc: '适用于 Windows 10 / 11 64位操作系统，支持一键向导安装与桌面快捷方式。',
    windowsInstaller: '标准安装版 (Setup.exe)',
    windowsPortable: '免安装便携版 (.zip)',
    macTitle: 'macOS 客户端',
    macDesc: '适配 Apple Silicon (M1/M2/M3/M4) 与 Intel 架构芯片。',
    macComingSoon: '适配就绪 / 即将上线',
    linuxTitle: 'Linux / 极客模式',
    linuxDesc: '支持 Docker 容器化部署与命令行 CLI 运行，适配无头服务器与开发者环境。',
    quickStartTitle: '一键接入配置助手',
    quickStartDesc: '客户端安装完成后，只需填入本站 API 地址与您的专属密钥，即可畅享智能体极速调用。',
    step1: '1. 安装并启动魔丸客户端',
    step2: '2. 复制下方本站接口地址与 API Key',
    step3: '3. 粘贴至客户端设置，立即开启智能体对话与任务',
