export default {
    accounts: {
      ...
      upstreamBilling: {
        trustWarning: '此倍率由上游站点针对当前 API Key 自行声明。英国api.cc 无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。',
      ...
      usageWindow: {
        ...
        grokRequests: '请求',
        grokTokens: 'Token',
        grokFreeQuota24hHint: '按本平台近 24 小时本地 Token 用量估算（上限 {limit}）',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个上游中继实例的场景。',
