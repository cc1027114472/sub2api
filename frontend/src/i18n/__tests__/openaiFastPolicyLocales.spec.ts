...
describe('OpenAI Fast/Flex policy locale keys', () => {
  it('exposes user scope copy at the runtime zh path', () => {
    expect(zh.admin.settings.openaiFastPolicy).toMatchObject({
      userIds: '指定用户',
      userIdsHint: '输入任意邮箱关键词进行模糊搜索。留空表示对全部 英国api.cc 用户生效；选中用户的 API Key 请求优先匹配用户规则。',
  ...
  it('exposes user scope copy at the runtime en path', () => {
    expect(en.admin.settings.openaiFastPolicy).toMatchObject({
      ...
      userIdsHint: 'Type any part of a user email to search. Leave empty to apply to all 英国api.cc users. Selected users match requests from their API keys and take precedence over global rules.',
